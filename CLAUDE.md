...
- `meta.json` carries the whole ecology palette: the `species` list with its colours, and since shot S2 an `overlays` array of `{name, lo, hi}` sRGB ramp ends (plus `mid` on `traits` and `burnt` on `fire`). Since shot S10 there is an eighth row, `water`, and it is the only one that also carries a `scale` — `{lo, hi, unit, curve}`, the ponded-depth ramp in millimetres, `curve` being `log10` — because ponded depth is the one overlay whose range is in no other published place; the top is the deepest water the site's ground can hold, rounded up to a decade, and the row has no `scale` at all when `hydro.enabled` is false. Surface media and buildings are not in it — they are scene geometry and belong to the bundle's renderer. Adding a `meta.json` key does not bump `format_version`.
