...
| `09_fire_t17100_top.png` **shown** | Tick with the most patches burning (17100): burning orange, burnt charcoal, the rest material | **PASS on the overlay, FAIL on the subject — the tick is now doubly wrong.** The overlay draws the material colour with trunks and animals on top, and nothing else: there is no orange, because no patch is burning at this tick, and since ecosim shot G4c there is no charcoal either, because there is not one burnout event between ticks 17000 and 17100. 17100 was picked when it was the snapshot with the most patches alight; on this run the peak is 6 patches at tick **8924**. Moving the tick means editing `scripts/shots.mjs` and adding a reference, which E6 was not asked to do, so it is recorded here and in `REACCEPT-E6.md`. The fire palette itself is gated by `overlays.spec.ts`, not by this picture. |
